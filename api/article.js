...
function articleHtml(item, relatedItems) {
  const title = clean(item.name) || "無題の記事";
  const canonical = `${ORIGIN}${articlePath(item)}`;
  const categories = cmsCategories(item);
  const parentUrl = categories.includes("リーク") ? "/leaks/" : "/articles/";
  const parentName = categories.includes("リーク") ? "リーク・未確認情報" : "最新記事";
  ...
  const sources = externalSources(rawBody);
  const toc = prepared.toc.length
...
<meta name="description" content="${escapeHtml(summary)}"><meta name="robots" content="index,follow,max-image-preview:large">
...
<script type="application/ld+json">${escapeJson(articleSchema)}</script><script type="application/ld+json">${escapeJson(breadcrumbSchema)}</script>
...
<article><header class="seo-page-hero cms-article-hero"><div class="seo-label-row"><span class="seo-label ${categories.includes("リーク") ? "leak" : "analysis"}">${escapeHtml(status)}</span>${categories.slice(0, 3).map((category) => `<span class="seo-label">${escapeHtml(category)}</span>`).join("")}</div><h1>${escapeHtml(title)}</h1><p class="cms-article-summary">${escapeHtml(summary)}</p><div class="cms-visible-dates"><span>公開日：<time datetime="${published}">${publishedLabel}</time></span>${modifiedLabel && modifiedLabel !== publishedLabel ? `<span>最終更新：<time datetime="${modified}">${modifiedLabel}</time></span>` : ""}<span>執筆・確認：<a href="/authors/editorial-team/">GTA6インフォ編集部</a></span></div></header>
...
<footer class="seo-footer"><div class="seo-container seo-footer-grid"><div><strong>GTA6インフォ</strong><p>公式情報と未確認情報を分けて整理する非公式ファンサイトです。</p></div><nav><a href="/articles/">最新記事</a><a href="/release/">発売情報</a><a href="/leaks/">リーク・未確認情報</a><a href="/editorial-policy/">編集・掲載方針</a><a href="/source-policy/">出典・引用方針</a><a href="/contact/">お問い合わせ</a></nav></div></footer></body></html>`;
}
...
export default async function handler(request, response) {
  ...
  try {
    ...
    const related = allItems
      ...
      .slice(0, 4);
    const html = articleHtml(item, related);
