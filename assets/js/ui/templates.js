...
function routeTarget(route){if(route==="map")return {href:"/map/",attr:""};if(route.startsWith("region/"))return {href:`/map/${route.slice(7)}/`,attr:""};return {href:`#${route}`,attr:`data-route="${route}"`}}
...
function newsCard(route,type,badge,title,desc,visual){const badgeClass=type==="official"?"badge-official":type==="db"?"badge-db":type==="guide"?"badge-guide":"";return `<a class="news-card filterable-news" data-type="${type}" href="#${route}" data-route="${route}"><span class="news-visual ${visual}"></span><span><span class="news-meta"><span class="badge ${badgeClass}">${badge}</span></span><h3>${title}</h3><p>${desc}</p></span></a>`}
