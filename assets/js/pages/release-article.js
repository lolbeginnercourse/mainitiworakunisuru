const releaseArticleSections=[
  ["official","現在確認できる公式情報"],
  ["date","発売日は2026年11月19日"],
  ["platforms","対応機種"],
  ["preorder","予約状況と国内価格"],
  ["bonus","予約特典"],
  ["package","パッケージ版と事前ダウンロード"],
  ["capacity","正式な必要容量は未発表"],
  ["space","発売前に確保したい空き容量"],
  ["ps5-storage","PS5で必要な保存場所"],
  ["xbox-storage","Xbox Series X|Sで必要な保存場所"],
  ["pc","PC版は未発表"],
  ["check","予約前に確認すること"],
  ["sources-official","公式参照先"]
];
...
function renderReleaseArticle(){
  ...
  return `<div class="container page-section release-article"><div class="article-layout"><article class="article-main"><a class="nav-back" href="#categories" data-route="categories">‹ 情報カテゴリへ戻る</a><div class="article-kicker"><span class="badge badge-official">公式情報</span><span class="badge">購入前ガイド</span></div><h1>GTA6の発売日・対応機種・予約状況｜必要容量と快適な空き容量</h1><div class="article-meta"><span>最終更新：2026年7月14日</span><span>確認：Rockstar Games・PlayStation Store・Xbox Store</span><span>ネタバレ：なし</span></div><div class="article-summary release-lead"><strong>現在の結論</strong><p>発売予定日は<strong>2026年11月19日</strong>、対応機種は<strong>PlayStation 5とXbox Series X|S</strong>です。予約受付は2026年6月25日に始まっています。</p><p>正式な必要容量はまだ発表されていません。発売前の準備目安は最低250GB、余裕を持つなら300〜350GB以上ですが、これらは公式容量ではありません。</p></div><nav class="release-mobile-toc" aria-label="この記事の目次"><strong>この記事の内容</strong><ol>${releaseArticleSections.map(([id,title])=>`<li><button type="button" class="toc-link" data-scroll-target="release-${id}">${title}</button></li>`).join("")}</ol></nav><div class="article-body">${body}</div><section class="page-section">${sectionHeading("次に見るページ")}<div class="data-list">${dataCard("confirmed","公式発表一覧","公式に公開されている情報をまとめて確認")}${dataCard("latest","最新情報","更新された内容を確認")}${dataCard("search","サイト内検索","人物や地域名で探す")}</div></section></article><aside class="article-side"><nav class="panel toc"><strong>この記事の内容</strong><ol>${releaseArticleSections.map(([id,title])=>`<li><button type="button" class="toc-link" data-scroll-target="release-${id}">${title}</button></li>`).join("")}</ol></nav><a class="notice" href="#sources" data-route="sources"><span class="notice-icon">${ICONS.info}</span><span><strong>出典・確認方針</strong><small>公式情報と準備目安を区別</small></span><span class="chevron">›</span></a></aside></div></div>`;
}
