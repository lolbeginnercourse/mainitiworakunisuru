...
function bindSearch(){
  ...
  const draw=()=>{const q=(input.value||"").trim().toLowerCase();const terms=q.split(/\s+/).filter(Boolean);const rows=items.filter(item=>(activeType==="all"||item.type===activeType)&&terms.every(term=>(`${item.title} ${item.desc} ${item.keys}`).toLowerCase().includes(term)));status.textContent=q?`「${q}」の検索結果 ${rows.length}件`:`全コンテンツ ${rows.length}件`;results.innerHTML=rows.length?rows.map(item=>`<a class="category-page-card" href="#${item.route}" data-route="${item.route}"><span><span class="badge">${item.typeLabel}</span><h3>${item.title}</h3><p>${item.desc}</p></span><span class="chevron">›</span></a>`).join(""):`<div class="empty-state"><strong>一致する情報がありません</strong><p>対象を一語にするか、カテゴリの絞り込みを「すべて」に戻してください</p><div class="empty-actions">${routeLink("categories","カテゴリを見る","button button-secondary")}${routeLink("sitemap","サイトマップ","button button-secondary")}</div></div>`};
