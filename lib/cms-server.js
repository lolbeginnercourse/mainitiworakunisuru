...
export async function fetchCmsContent(id) {
  ...
  if (!value || withdrawnContentIds.has(value)) {
  ...
  const { endpoint } = cmsConfig();
  try {
    return await cmsRequest(`${encodeURIComponent(endpoint)}/${encodeURIComponent(value)}`);
  } catch (error) {
    if (error?.status !== 404) throw error;
    const contents = await fetchCmsContents(100);
    const matched = contents.find((item) => clean(item.slug) === value);
