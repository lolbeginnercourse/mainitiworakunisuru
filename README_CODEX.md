...
Current launch state: menu scanning is live. The browser flag is `window.MENU_SAFE_LENS_LAUNCH_PAUSED = false` in `assets/js/config.js`, and the serverless API has `ANALYSIS_PAUSED = false` in `api/analyze-menu.js`.
